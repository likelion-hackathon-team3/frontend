...
import { TriangleAlert, ArrowRight, Loader2, Moon, Footprints, HeartPulse, Watch } from 'lucide-react'
...
export default function AnalysisResultScreen() {
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([fetchAnalysis(), fetchWearableData()]).then(([res, wearable]) => {
      if (cancelled) return
  ...
  const riskLabel = state.data ? RISK_LABEL[state.data.riskLevel] || state.data.riskLevel : ''
  const transition = state.data ? formatTransitionLabel(state.data.transitionType) : { short: '-', full: '' }
            ...
            <div className={`rounded-2xl border p-5 mb-6 ${risk.bg} ${risk.border}`}>
              <div className="flex items-center gap-2">
                ...
                <p className="text-base font-bold text-ink">
                  이번 {transition.short} 전환은 '{riskLabel}' 등급이에요
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-5 mb-6">
              <div className="bg-card rounded-2xl border border-lavender/10 p-5">
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <Cell label="전환 유형" value={transition.short} sub={transition.full} valueClass="text-lavender-deep" />
                  <Cell label="최근 연속 근무일수" value={`${state.data.consecutiveDays}일`} sub="(오늘 포함)" />
                  <Cell label="다음 근무까지" value={formatShiftMinutes(state.data.nextShiftMinutes)} />
                  <Cell label="실제 활용 가능시간" value={formatHoursLabel(state.data.availableHours)} valueClass="text-sage" />
                </div>

                <div className="bg-bg rounded-xl p-4">
                  <p className="text-xs text-muted mb-1">전환 위험도</p>
                  <div className={`flex items-center gap-1.5 font-bold text-sm ${risk.text}`}>
              ...
              <div className="bg-card rounded-2xl border border-lavender/10 p-5">
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  ...
                  />
                  <Cell label="종합 회복 상태" value={state.data.currentCondition.recoveryStatus} valueClass="text-coral" />
                </div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  ...
                  <MiniStat
                    icon={Footprints}
                    label="활동량"
                    value={state.wearable ? `${state.wearable.activitySteps.toLocaleString()} 걸음` : '-'}
                  />
                  <MiniStat icon={HeartPulse} label="안정시 심박수" value={state.wearable ? `${state.wearable.heartRate} bpm` : '-'} />
                  <MiniStat icon={Watch} label="착용 기기" value="스마트워치" />
                </div>
              </div>
            </div>

            <div className="bg-lavender-deep/5 border border-lavender/10 rounded-2xl p-5 flex items-center justify-end">
              <button
                type="button"
